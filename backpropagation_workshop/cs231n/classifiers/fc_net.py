...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss, dl_dout2 = softmax_loss(out2, y)
        reg_loss = (
            loss
            + self.reg / 2 * np.square(self.params['W1']).sum()
            + self.reg / 2 * np.square(self.params['W2']).sum()
        )
        
        dl_dout1, dl_dW2, grads['b2'] = affine_backward(dl_dout2, cache2)
        grads['W2'] = dl_dW2 + self.reg * self.params['W2']
        _, dl_dW1, grads['b1'] = affine_relu_backward(dl_dout1, cache1)
...
#             print(outs[-1].shape, self.params['W' + str(i)].shape, self.params['b' + str(i)].shape)
...
#                     outs.append(out)
        ...
        loss, dl_dout = softmax_loss(scores, y)
        for i in range(self.num_layers)[::-1]:
            ...
            loss += self.reg / 2 * np.square(self.params['W' + str(i)]).sum() 
